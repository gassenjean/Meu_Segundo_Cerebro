---
criado: 2025-11-18T10:49:27-03:00
atualizado: 2025-11-19T19:13:19-03:00
---
# 📚 MOC: CONHECIMENTO

**Map of Content - Base de Conhecimento Pessoal**

**Criado:** 17/Jan/2025
**Última atualização:** 19/Nov/2024
**Total de itens:** 8

---

## 🎯 O QUE É ESTA ÁREA

Sua base de conhecimento permanente. Aqui você armazena:
- Conceitos aprendidos
- Resumos de livros/artigos
- Conhecimento técnico
- Insights pessoais
- Referências importantes

**Princípio:** Conhecimento que você quer MANTER para sempre.

---

## 📂 ESTRUTURA

### IA_e_Tecnologia/
IA, programação, ferramentas, automação, Claude Code

**Conteúdo:**
- [[IA_e_Tecnologia/Claude_Code_Templates_Repositorio.md|Claude Code Templates]]
- [[IA_e_Tecnologia/Clone_IA_Framework_Alan.md|Clone IA Framework (213KB)]]
- [[IA_e_Tecnologia/Aurora_ChatGPT_Sistema.md|Aurora ChatGPT Sistema (109KB)]]
- [[IA_e_Tecnologia/Alan_IA_Config.md|Alan IA Config]]

### Desenvolvimento_Pessoal/
Produtividade, hábitos, mindset, TDAH, autoconhecimento

**Conteúdo:**
- [[Desenvolvimento_Pessoal/Modelagem_Mental_Alan.md|Modelagem Mental]]
- [[Desenvolvimento_Pessoal/Zona_Genialidade_Alan.md|Zona de Genialidade]]

### Produtividade/
PKM, segundo cérebro, MOCs, Obsidian

**Conteúdo:**
- [[Produtividade/MOC_PKM_Segundo_Cerebro_Alan.md|MOC PKM & Segundo Cérebro]]

### Negocios/
Marketing, vendas, empreendedorismo, cultura

**Conteúdo:**
- (em construção)

---

## 📋 COMO USAR

### Adicionando Conhecimento

1. **Identifique a subcategoria** (DevPessoal, Tech, Negocios)
2. **Nomeie corretamente:**
   ```
   Conhecimento_[Sub]_[Topico].md

   Exemplos:
   Conhecimento_Tech_Prompt_Engineering.md
   Conhecimento_DevPessoal_TDAH_Foco.md
   Conhecimento_Negocios_Marketing_Digital.md
   ```
3. **Crie o arquivo na pasta certa**
4. **Atualize este MOC** (adicione link na seção)

### Template de Nota

```markdown
# [Título do Conhecimento]

**Fonte:** [Livro/Artigo/Curso/Experiência]
**Data:** [Quando aprendeu]
**Tags:** [tag1, tag2]

---

## Conceito Principal

[Resumo do conceito em 2-3 parágrafos]

## Pontos Chave

- Ponto 1
- Ponto 2
- Ponto 3

## Aplicação Prática

[Como usar isso no dia a dia]

## Links Relacionados

- [[Outro_Conhecimento]]
- [[Projeto_Relacionado]]

---

**Última revisão:** [Data]
```

---

## 🔍 BUSCA RÁPIDA

### Por Tema

**IA e Automação:**
- [[IA_e_Tecnologia/Claude_Code_Templates_Repositorio.md|Claude Code Templates]]
- [[IA_e_Tecnologia/Clone_IA_Framework_Alan.md|Clone IA Framework]]
- [[IA_e_Tecnologia/Aurora_ChatGPT_Sistema.md|Aurora ChatGPT]]

**Produtividade:**
- [[Produtividade/MOC_PKM_Segundo_Cerebro_Alan.md|PKM & Segundo Cérebro]]

**Desenvolvimento Pessoal:**
- [[Desenvolvimento_Pessoal/Modelagem_Mental_Alan.md|Modelagem Mental]]
- [[Desenvolvimento_Pessoal/Zona_Genialidade_Alan.md|Zona de Genialidade]]

### Por Fonte

**Livros:**
- (adicionar links)

**Cursos:**
- (adicionar links)

**Experiências:**
- (adicionar links)

---

## 📊 ESTATÍSTICAS

```
📁 Total de notas: 8
📂 IA e Tecnologia: 4
📂 Desenvolvimento Pessoal: 2
📂 Produtividade: 1
📂 Negócios: 0
📅 Última adição: 19/Nov/2024
```

---

## 🔗 LINKS

### MOCs Relacionados
- [[00_SISTEMA/MOCs/MOC_SEGUNDO_CEREBRO_MASTER.md]] - MOC principal
- [[_MOC_Aprendizado.md]] - Cursos em andamento
- [[_MOC_Projetos.md]] - Projetos ativos

### Recursos
- [[04_RECURSOS/TEMPLATES/]] - Templates de notas
- [[00_SISTEMA/PADROES/NOMENCLATURA.md]] - Como nomear

---

## ✅ MANUTENÇÃO

### Semanal
- [ ] Revisar notas criadas na semana
- [ ] Atualizar este MOC com novos links
- [ ] Verificar se nomenclatura está correta

### Mensal
- [ ] Consolidar notas relacionadas
- [ ] Revisar e atualizar estatísticas
- [ ] Arquivar notas desatualizadas

---

**Criado:** 17/Jan/2025
**Versão:** 1.0

**CONHECIMENTO É PODER! 📚**
