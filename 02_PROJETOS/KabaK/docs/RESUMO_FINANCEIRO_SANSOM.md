---
criado: 2026-01-14T15:45:00-03:00
atualizado: 2026-01-14T15:45:00-03:00
tipo: resumo_executivo_financeiro
destinatario: Sansom
prioridade: critica
---

# RESUMO FINANCEIRO EXECUTIVO - Para Sansom

**Data:** 14/Jan/2026
**Projeto:** Sociedade KabaK com você (50% lucros) + Jean/Família (50% lucros)
**Decisão:** 51% você (voto minerva) / 49% Jean (família)

---

## 👥 ESTRUTURA SOCIEDADE

```
SANSOM:                          51% decisão + 50% lucros
JEAN (FAMÍLIA):                  49% decisão + 50% lucros
  ├── Jean: Dono/CEO KabaK atual
  ├── Gassen: Filho Jean, criador marca
  └── Kris: Esposa Gassen, co-criadora, produto

DIVISÃO INTERNA FAMÍLIA: A definir com Dr. Alexandre
```

**Explicação:**
- **51%:** Você tem voto de minerva (desempate)
- **50%:** Lucros divididos igualmente
- Jean representa lado família (Jean+Gassen+Kris)

---

## 💰 O INVESTIMENTO

```
SEU INVESTIMENTO (50%):          R$ 1.300.000

QUANDO PRECISA:
├── Pré-operacional (Jan-Abr):   R$   990.000  [maioria aqui]
└── Operacional (Mai-Jun):       R$   310.000  [pequeno]

COMO USAR:
├── Tecido China (seu):          R$   780.000  [você controla]
├── Produção KabaK:              R$   640.000  [fábrica Jean/CEO]
├── Marketing Titanium:          R$   195.000  [agência]
└── Operação/Outros:             R$   285.000  [estrutura]
```

---

## 📈 O RETORNO

### ANO 1 (12 meses):
```
VENDAS:                          177.000 kits
FATURAMENTO:                     R$ 22.833.000
LUCRO TOTAL:                     R$  7.503.875

SEU LUCRO (50%):                 R$  3.751.938 ✅
LUCRO JEAN/FAMÍLIA (50%):        R$  3.751.938 ✅
```

### ROI (RETORNO SOBRE INVESTIMENTO):
```
Você investiu:                   R$ 1.300.000
Você lucrou (ano 1):             R$ 3.751.938

ROI = 288% ✅✅✅

Ou seja: Para cada R$ 1 investido, você ganha R$ 2,88 de volta no primeiro ano.
```

---

## ⏱️ QUANDO RECUPERA O DINHEIRO?

```
MÊS 1 (Mai):     -R$   59.453  [prejuízo - lançamento]
MÊS 2 (Jun):     -R$    4.960  [prejuízo - escalando]
MÊS 3 (Jul):     +R$  131.274  [POSITIVO! Break-even ✅]
MÊS 4 (Ago):     +R$  237.711  [lucro crescendo]
MÊS 5 (Set):     +R$  319.452  [lucro forte]
MÊS 6 (Out):     +R$  406.292  [consolidado]

TOTAL 6 MESES:   +R$ 1.030.316

RECUPEROU INVESTIMENTO: Mês 4-5 (Agosto-Setembro) ✅
DEPOIS: Só lucro puro! R$ 650k-900k/mês
```

---

## 💵 FLUXO DE CAIXA MENSAL

### Primeiros 6 Meses:
```
Mês 1 (Mai):  Vende 1.000 kits  = R$   129k receita → Prejuízo R$  59k
Mês 2 (Jun):  Vende 3.000 kits  = R$   387k receita → Prejuízo R$   5k
Mês 3 (Jul):  Vende 8.000 kits  = R$ 1.032k receita → Lucro R$  131k ✅
Mês 4 (Ago):  Vende 12.000 kits = R$ 1.548k receita → Lucro R$  237k
Mês 5 (Set):  Vende 15.000 kits = R$ 1.935k receita → Lucro R$  319k
Mês 6 (Out):  Vende 18.000 kits = R$ 2.322k receita → Lucro R$  406k
```

### Depois (Meses 7-12):
```
Vendendo 20.000 kits/mês:

Receita mensal:                  R$ 2.580.000
Custos todos:                    R$ 1.666.500
LUCRO MENSAL:                    R$   913.500 ✅

SEU LUCRO MENSAL (50%):          R$   456.750
```

**Resumo:** Depois do mês 6, você tira ~R$ 456k/mês de lucro (50%).

---

## 📊 DE ONDE VEM O LUCRO?

### POR KIT VENDIDO:
```
PREÇO VENDA:                     R$ 129,00

(-) CUSTOS:
├── Tecido China (seu):          R$  30,00
├── Fabricação KabaK (Jean CEO): R$  15,00
├── Embalagem/tags:              R$   3,00
├── Frete/logística:             R$  15,48
├── Gateway pagamento:           R$   3,87
└── Impostos (MG 2,5%):          R$   3,23
TOTAL CUSTOS:                    R$  70,58

LUCRO POR KIT:                   R$  58,42 ✅
MARGEM:                          45,3%
```

**Exemplo:** Vendendo 20.000 kits/mês
- Lucro bruto: 20.000 x R$ 58,42 = R$ 1.168.400
- Custos fixos (marketing, equipe): R$ 255.000
- **Lucro líquido: R$ 913.400/mês**

---

## 🎯 CENÁRIOS

### CONSERVADOR (Projeção base):
```
Vendas ano 1:                    177.000 kits
Faturamento:                     R$ 22.833.000
Lucro:                           R$  7.503.875
SEU LUCRO (50%):                 R$  3.751.938
ROI:                             288%
```

### OTIMISTA (+30% vendas):
```
Vendas ano 1:                    230.000 kits
Faturamento:                     R$ 29.670.000
Lucro:                           R$ 12.000.000+
SEU LUCRO (50%):                 R$  6.000.000+
ROI:                             461%+
```

### PESSIMISTA (-30% vendas):
```
Vendas ano 1:                    124.000 kits
Faturamento:                     R$ 15.996.000
Lucro:                           R$  3.000.000+
SEU LUCRO (50%):                 R$  1.500.000+
ROI:                             115%
```

**Conclusão:** Mesmo no pior cenário, você tem ROI 115% (mais que dobra dinheiro).

---

## ⚙️ NÚMEROS IMPORTANTES VALIDAR

### 🔴 URGENTE - Você precisa confirmar:

**1. CUSTO TECIDO CHINA:**
```
Estamos usando:                  R$ 30,00 por kit (3 peças)
Ou seja:                         R$ 10,00 por peça

Isso inclui:
├── Preço tecido na China:       R$ X
├── Frete China → Brasil:        R$ X
├── Impostos importação:         R$ X
└── TOTAL LANDED:                R$ 10/peça? ✅ ou ❌

SE CUSTO FOR MAIOR:
- Exemplo: R$ 12/peça = R$ 36/kit (em vez de R$ 30)
- Impacto: Margem cai de 45% para 40%
- Lucro ano 1: R$ 5,5M (em vez de R$ 7,5M)
- AINDA VIÁVEL, mas menos rentável

SE CUSTO FOR MENOR:
- Exemplo: R$ 8/peça = R$ 24/kit
- Impacto: Margem sobe para 50%+
- Lucro ano 1: R$ 9M+
- MELHOR AINDA!
```

**ACTION:** Confirmar custo real até 21/Jan!

---

**2. CAPACIDADE INVESTIMENTO:**
```
Você tem R$ 1.300.000 disponível (50%)?

OPÇÕES:

A) Tem integral:
   ✅ Aporta tudo início
   ✅ Máxima segurança
   ✅ Não depende de lucros curto prazo

B) Tem parcial:
   ⚠️ Aporta % maior início (ex: 60% = R$ 780k)
   ⚠️ Ajusta depois com lucros
   ⚠️ Renegocia participação % (temporário)

C) Não tem:
   ❌ Precisa buscar outra solução
   ❌ Investidor adicional?
   ❌ Crédito/financiamento?
```

**ACTION:** Conversar com Patrícia e confirmar até 21/Jan!

---

## 🎁 DISTRIBUIÇÃO DE LUCROS

### OPÇÃO A: Reinveste 50% / Distribui 50%
```
Lucro ano 1:                     R$ 7.503.875
Você recebe (50% de 50%):        R$ 1.875.969 💰
Fica na empresa (crescer):       R$ 1.875.969

VANTAGEM: Equilibrado (você ganha + empresa cresce)
```

### OPÇÃO B: Reinveste 70% / Distribui 30%
```
Lucro ano 1:                     R$ 7.503.875
Você recebe (50% de 30%):        R$ 1.125.581 💰
Fica na empresa (crescer):       R$ 3.751.938

VANTAGEM: Crescimento rápido (menos pra você ano 1, mais depois)
```

### OPÇÃO C: Reinveste 30% / Distribui 70%
```
Lucro ano 1:                     R$ 7.503.875
Você recebe (50% de 70%):        R$ 2.626.356 💰
Fica na empresa (crescer):       R$ 1.125.581

VANTAGEM: Máximo lucro ano 1 (recupera investimento + 102%)
```

**Qual você prefere?** Decidir no contrato social.

---

## 🚨 RISCOS E COMO EVITAR

### RISCO 1: Tecido China atrasa
```
PROBABILIDADE: Média (20-30%)
IMPACTO: Alto (adia lançamento 1-2 meses)

SOLUÇÃO:
├── Pedir AGORA (até 21/Jan) ✅
├── Buffer 30 dias no cronograma
└── Fornecedor backup nacional (mais caro)
```

### RISCO 2: Vendas abaixo do esperado
```
PROBABILIDADE: Baixa (15%)
IMPACTO: Médio (break-even demora mais)

SOLUÇÃO:
├── Modelo Atara já validado ✅
├── Começar conservador (1k kits mês 1)
├── Ajustar marketing conforme conversão
└── Mesmo -30% vendas = lucro R$ 1,5M/ano você
```

### RISCO 3: Custo China maior que esperado
```
PROBABILIDADE: Média (25%)
IMPACTO: Médio (margem cai ~5%)

SOLUÇÃO:
├── Validar custo REAL agora ✅
├── Testar amostras (qualidade = preço Atara?)
└── Mesmo +20% custo = ainda rentável
```

### RISCO 4: Fábrica Jean limita produção
```
PROBABILIDADE: Baixa (10%)
IMPACTO: Médio (não consegue escalar rápido)

SOLUÇÃO:
├── Capacidade atual: 80k peças/mês ✅
├── Pode expandir até 240k (3x)
├── Costureiras externas (rede pronta)
└── Exclusividade 100% garantida no contrato
```

---

## ✅ POR QUE ISSO VAI DAR CERTO?

### 1. MODELO VALIDADO
```
Atara (concorrente):
├── Mesmo produto (kit fitness feminino)
├── Mesmo preço (R$ 119-129)
├── Mesma agência (Titanium)
└── Resultado: R$ 10M/mês em 1 ano ✅

Nós vamos replicar isso + vantagens competitivas.
```

### 2. VANTAGENS COMPETITIVAS
```
vs Atara:
├── Custo 30-40% menor (seu tecido China)
├── Entrega 50% mais rápida (fábrica KabaK MG)
├── Capacidade ilimitada (Atara é limitada)
└── Experiência 7+ anos (Jean CEO + família)
```

### 3. TIMING PERFEITO
```
Mercado fitness feminino:
├── Crescendo 20% ao ano
├── Público C subatendido
├── E-commerce explodindo
└── Concorrentes não conseguem atender demanda
```

### 4. PARCERIA FORTE
```
Você (Sansom):               China, importação, financeiro
Jean (Família):              CEO KabaK, produção, marca
├── Jean:                    Dono/CEO, fábrica experiente
├── Gassen:                  Criador marca, gestão, inovações
└── Kris:                    Co-criadora, produto, processo
Titanium (agência):          Marketing validado

= Time completo, complementar, experiente
```

---

## 🎯 PRÓXIMOS PASSOS (VOCÊ)

### ESTA SEMANA (14-21/Jan):

**1. FALAR COM PATRÍCIA** ✅
- Mostrar este documento
- Explicar investimento (R$ 1,300M = 50%)
- Explicar retorno (R$ 3,75M ano 1 = 288% ROI)
- Explicar estrutura: Você 50% + Jean/Família 50%
- Decidir: PF ou PJ? Integral ou parcelado?

**2. PEDIR TECIDO CHINA** 🔥🔥🔥
- Contatar fornecedores (pelo menos 3)
- Pedir cotações (preço, prazo, qualidade)
- Negociar (alvo R$ 10/peça, máx R$ 12)
- **FECHAR PEDIDO até 21/Jan**
- Confirmar chegada Mar-Abr/2026

**3. VALIDAR NÚMEROS** ✅
- Custo real tecido China (R$ 30/kit OK?)
- Capacidade investimento (R$ 1,300M = 50% OK?)
- Forma de aporte (integral ou parcelado?)

---

## 📞 QUALQUER DÚVIDA

**Gassen está à disposição:**
- Telefone: [número]
- WhatsApp: [número]
- Email: [email]

**Documentos completos:**
- Este resumo (2 páginas - rápido)
- Planilha detalhada 12 meses (20 páginas - completo)
- Checklist suas ações (15 páginas - passo a passo)

---

## 💬 MENSAGEM FINAL

Sansom,

Você está entrando em um negócio que:
- **Modelo validado** (Atara faz R$ 10M/mês)
- **Investimento claro** (R$ 1,326M)
- **Retorno forte** (R$ 3,8M ano 1 = 288% ROI)
- **Payback rápido** (4-5 meses)
- **Time completo** (você + Gassen + Jean + Titanium)

**Sua parte crítica:**
1. Tecido China (seu diferencial)
2. Gestão financeira (51% = controle)
3. Decisão final (1% casos de impasse)

**Risco controlado:**
- Mesmo cenário pessimista (-30% vendas) = lucro R$ 1,5M
- Auditoria obrigatória (transparência total)
- Contrato claro (saída sem briga)

**Você mencionou na reunião:**
- "Nunca fiz sociedade, quero fazer certo" ✅
- "Não quero briga no futuro" ✅
- "Vamos preparar o pior cenário" ✅

**Estamos fazendo exatamente isso.**

Agora é **executar:**
1. Patrícia OK? ✅
2. Tecido pedido? ✅
3. Números validados? ✅

**Vamos fazer isso dar certo!** 🚀

---

**Documento criado:** 14/Jan/2026
**Para:** Sansom (leitura rápida 10 minutos)
**Baseado em:** Planilha completa 12 meses + Reunião 14/Jan
**Ação:** Validar e confirmar até 21/Jan
