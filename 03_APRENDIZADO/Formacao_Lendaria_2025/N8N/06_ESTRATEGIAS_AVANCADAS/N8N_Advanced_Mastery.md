...
const customerContext = await getUnifiedCustomerProfile(items[0].json.customer_id);
...
return [{ json: { score: spiritualInterestScore, readiness: readinessForStudy } }];
```
...
```json
{
  "test_framework": {
    "message_variations": {
      "evangelical": ["Direct scripture", "Personal testimony", "Question-based"],
...
const optimizationOpportunities = await identifyOptimizations(performanceMetrics);
...
return [{ json: { metrics: performanceMetrics, optimizations: optimizationOpportunities } }];
```
