---
description: Contexto Cultivo Medicinal - Dr. Green (conhecimento especializado, pesquisa, análise)
argument-hint: [opcional] "pesquisa" | "analise" | "status"
---

# Dr. Green - Contexto Cultivo Medicinal

Ativa contexto **Dr. Green** - especialista em cultivo medicinal.

---

## 🎯 QUANDO USAR

Use `/dr-green` quando trabalhar em:
- ✅ Cultivo medicinal (conhecimento)
- ✅ Pesquisa e estudo do setor
- ✅ Análises específicas
- ✅ Legislação e compliance

**ISOLAMENTO:** Este contexto carrega APENAS material Cultivo. Zero vazamento para outros domínios.

---

## 📚 CONTEXTO CARREGADO

**Material base:**
- **MOC Master:** `01_CONHECIMENTO/Cultivo_Medicinal/_MOC_Cultivo_Medicinal.md`
- **Status Atual:** `01_CONHECIMENTO/Cultivo_Medicinal/Ciclos/Cultivo_Status_Dia12_31DEZ2025.md`
- **Agente Dr. Green:** `04_RECURSOS/PROMPTS/Agentes_Sistema/PROMPT_AGENTE_DR_GREEN.md`
- **26 arquivos:** Protocolos, Sistema, Diagnóstico, Histórico

**Expertise:**
- Cultivo medicinal orgânico (Solo Vivo)
- 8 protocolos de produtos (EM-1, PSB POW, Sollem, LMX, Off Neem, etc)
- Diagnóstico (deficiências, pragas, soluções)
- Sistema técnico (Blumat, SCROG, fotoperíodo)
- Cronobiologia vegetal e microbiologia

---

## 🚀 COMANDOS DISPONÍVEIS

```bash
# Ativar contexto Cultivo
/dr-green

# Pesquisa específica
/dr-green pesquisa

# Análise do setor
/dr-green analise

# Status geral
/dr-green status
```

---

## 📊 STATUS ATUAL (31/12/2025)

**Ciclo 2 - Dia 12 Floração:**
- 🌸 Flip: 19/12/2025
- 🌱 Fotoperíodo: 12/12
- ✅ Moscas: ZERO (erradicadas!)
- ✅ Verde saudável, SCROG denso
- 🌱 20 clones novos (dia 2 enraizamento)

**Aplicações HOJE:**
- ⚡ **LMX 0,5ml/L** às 18:00h (atrasado 12 dias)
- ⚡ **Off Neem 1ml/L** período escuro (preventivo)

**Próximas aplicações:**
- 10/01: EM-1 + Melaço
- 16/01: LMX + Off Neem (ÚLTIMA antes colheita)

---

## 🔗 INTEGRAÇÃO

**Com /mapa:**
```bash
/mapa        # Carrega índice
/dr-green    # Ativa contexto Cultivo
# Dr. Green sabe TODO material Cultivo!
```

**Com /coach:**
```bash
/coach foco "pesquisa cultivo"
# Coach carrega contexto Dr. Green
# Timebox 45 min focado APENAS em Cultivo
```

---

## ⚠️ ISOLAMENTO DE CONTEXTO

**Quando /dr-green está ativo:**
- ✅ Acesso: Material Cultivo, Dr. Green, pesquisas setoriais
- ❌ ZERO acesso: DeFi (Lucas), Tráfego (Pedro), IA (Alan), Produtividade (Elena)
- ❌ NÃO carrega: Cursos principais, projetos não-relacionados

**Benefício:**
- Economia: ~95% tokens (contexto muito específico)
- Foco: 100% Cultivo medicinal
- Conhecimento especializado disponível

---

## 📈 RESULTADO ESPERADO

**Uso conforme necessidade:**
- Conhecimento especializado Cultivo
- Pesquisas aprofundadas
- Análises do setor
- Compliance e legislação

**Prioridade:** Baixa (usar quando relevante ao trabalho)

---

## 💡 EXEMPLO DE USO

```bash
# Sessão Cultivo (quando necessário)
/mapa
/dr-green
/coach check-in

Coach: Resultado final hoje?
Você: Pesquisar legislação cultivo medicinal

Coach: Timebox 45 min. Contexto: Cultivo (Dr. Green).

# Foco 100% Cultivo
# Zero dispersão outros domínios

Dr. Green: Contexto Cultivo ativo.
Legislação medicinal: Material carregado.
Pesquisa específica?
```

---

**Criado:** 30/12/2025
**Versão:** 1.0
**Status:** ✅ Pronto para uso
**Prioridade:** ⭐ Baixa (conforme necessidade)

**CONTEXTO CULTIVO ATIVADO. CONHECIMENTO ESPECIALIZADO DISPONÍVEL! 🎯**
