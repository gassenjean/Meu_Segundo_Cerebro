...
var EmotionPickerSettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Add comma after").setDesc("Add a comma after pasted emotion").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useCommaInSeparator).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.useCommaInSeparator = value;
        yield this.plugin.saveSettings();
      }))
    );
    new import_obsidian3.Setting(containerEl).setName("Insert as link").setDesc("Insert emotion as a [[link]]").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.addAsLink).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.addAsLink = value;
        yield this.plugin.saveSettings();
      }))
    );
    new import_obsidian3.Setting(containerEl).setName("Insert as tag").setDesc("Insert emotion as a #tag").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.addAsTag).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.addAsTag = value;
        yield this.plugin.saveSettings();
      }))
    );
    new import_obsidian3.Setting(containerEl).setName("Capitalize").setDesc("Capitalize (useful if inserting emotion as link or tag)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.capitalize).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.capitalize = value;
        yield this.plugin.saveSettings();
      }))
    );
    new import_obsidian3.Setting(this.containerEl).setName("Override locale:").setDesc(
    ...
    this.plugin.settings.emotions.forEach((es) => {
      ...
      setting.addButton((btn) => {
        ...
        btn.onClick(() => {
          this.plugin.settings.emotions = this.plugin.settings.emotions.filter(
            (e) => e.id !== es.id
          );
          this.plugin.saveSettings();
          this.refresh();
